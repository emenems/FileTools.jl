...
writetsf(data,"../test/input/tsf_data.tsf",units=["degC","nm/s^2"],
...
function writetsf(data::DataFrame,fileout::String;
	...
	# copy only used columns (will be modified)
	...
	open(fileout,"w") do fid
		...
		for (i,v) in enumerate(channels)
			if length(channels) == length(decimal)
				dataout[v] = round.(dataout[v].*10^decimal[i])./10^decimal[i];
			elseif length(decimal) == 1
				dataout[v] = round.(dataout[v].*10^decimal[1])./10^decimal[1];
			end
		end
		for i = 1:nlines
